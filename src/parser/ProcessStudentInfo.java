...
public class ProcessStudentInfo {
	...
	public static void main(String[] args) throws ParserConfigurationException, SAXException, IOException {
		...
		seleniumStudent = xmlR.parseData(tag, pathSelenium);

		//Parse Data using parseData method and then store data into Qtp ArrayList.
		qtpStudent = xmlR.parseData(tag, pathQtp);
		...
		String storeData = Arrays.toString(map.toArray());
		ConnectDB connectDB1 = new ConnectDB();
		List<String> studentInfo = new ArrayList<>();
		try {

			//create table Studentdata in the databse
			connectDB1.Query("create table Studentdata(Studentdata Text);");
			//Store data into the table.
			connectDB1.InsertDataFromStringToMySql(storeData, "Studentdata", "Studentdata");
			//retrieving From database(below)
			studentInfo = connectDB1.readDataBase("Studentdata", "Studentdata");

		} catch (Exception e) {
			e.printStackTrace();

		}
		System.out.println("\nReading data from Studentdata table");
		for(String st:studentInfo){
